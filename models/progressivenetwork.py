...
from models.basicnetwork import conv_out_size
...
class ProgressiveNetwork(nn.Module):
    ...
    def encode(self, x):
        r"""Evaluate the network on the given input, one layer at a time, passing through
        gates as needed.
        """
        x = x.unsqueeze(1)
    ...
    def collate(self, inputs, labels):
        r"""
        """
        max_t = conv_out_size(self.currentnet.conv, max(i.shape[0] for i in inputs), 0)
